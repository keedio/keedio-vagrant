class ipa::server {
 ...
 $passwd=hiera(ipa_password)
  ...
  exec { 'ipa':
  command => "/usr/sbin/ipa-server-install -a $passwd --hostname=master.$set_subdomain.$set_domain -r $set_realm -p $passwd -n $set_subdomain.$set_domain  -U >& /root/ipa-install.log",
